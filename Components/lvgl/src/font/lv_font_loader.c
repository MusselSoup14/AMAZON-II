                          ...
                          uint32_t start, uint32_t * glyph_offset, uint32_t loca_count, font_header_bin_t * header)
{
    ...
    int cur_bmp_size = 0;
    unsigned int loca_count_index;
    
    for(loca_count_index = 0; loca_count_index < loca_count; ++loca_count_index) {
        lv_font_fmt_txt_glyph_dsc_t * gdsc = &glyph_dsc[loca_count_index];

        lv_fs_res_t res = lv_fs_seek(fp, start + glyph_offset[loca_count_index], LV_FS_SEEK_SET);
        if(res != LV_FS_RES_OK) {
            return -1;
        }
        ...
        int next_offset = (loca_count_index < loca_count - 1) ? glyph_offset[loca_count_index + 1] : (uint32_t)glyph_length;
        int bmp_size = next_offset - glyph_offset[loca_count_index] - nbits / 8;

        if(loca_count_index == 0) {
            gdsc->adv_w = 0;
            gdsc->box_w = 0;
            gdsc->box_h = 0;
            gdsc->ofs_x = 0;
