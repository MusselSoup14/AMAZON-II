...
static int clz_copy(unsigned int word)
{
  int nCnt = 0;
...
{  
    const int bit = word ? (32 - clz_copy(word)) : 0;
...
static block_header_t * block_locate_free(control_t * control, size_t size)
{
    ...
    if(block) {
        //printf("block_size(block) = %d, size = %d \r\n", block_size(block), size);
...
int test_ffs_fls()
{
    ...
    {
        printf("tlsf_fls[%d] = %d",index, tlsf_fls(index));
    }
#endif 
    ...
    if(rv) {
        ...
        printf("test_ffs_fls: %x ffs/fls tests Success.\n", rv);
    return rv;
}
#endif
