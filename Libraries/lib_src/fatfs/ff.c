		...
		if ((fp->fptr % SS(fp->fs)) == 0) {		/* On the sector boundary? */
			...
			if (cc) {							/* Read maximum contiguous sectors directly */
				if (csect + cc > fp->fs->csize)	/* Clip at cluster boundary */
				{
					...
					/* Read maximum contiguous clusters directly,by kkh  */
#define ADC_CONT_PATCH 
#ifdef ADC_CONT_PATCH
					DWORD prevclst = fp->clust;
					DWORD nextclst;
					while (cc + fp->fs->csize < (btr / SS(fp->fs)))
					{
						nextclst = get_fat(fp->fs, prevclst);
						if (nextclst == prevclst + 1)
						{
							cc += fp->fs->csize;
							fp->clust = prevclst = nextclst;
						}
						else
						{
							break;
